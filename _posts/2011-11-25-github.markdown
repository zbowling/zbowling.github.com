...
At SeatMe, we maintain a fork of [AFNetworking](https://github.com/seatme/AFNetworking) with a number of improvements that solve bugs. Early on it started out where we had made some [minor changes](https://github.com/AFNetworking/AFNetworking/pull/81) to fix some critical issues to get our code running. Our changes never made it upstream because the upstream maintainer didn't feel it was necessary saying the fix seemed to "complication for something that does not appear to be an issue for a vast majority of people". Since we can't get upstream to take our changes it means that we need to continue to maintain the changes in our fork to fix the bug. As we continue to innovate new features on top of those changes to support our app, our fork is growing in divergent. While we do [try](https://github.com/AFNetworking/AFNetworking/pull/125) to [send](https://github.com/AFNetworking/AFNetworking/pull/126) as many changes as we can upstream that we know may not conflict, we are the road to having a hard fork. When that happens, for the same reasons we use open source libraries, it's going to be advantageous for us to get other people out there to start using our code as well so we don't potentially have to support the fork on our own and get more testing done on the app.
...
This is all really suboptimal and can be really solved by changing the network tab.
